...
export function createAudio(): AudioModule {
  ...
  /** Pink (1/f) noise via Paul Kellett's filter — energy falls off with
   *  frequency, so it reads as soft distant rainfall instead of the harsh
   *  full-spectrum hiss of white noise. Used only for the rain loop; the
   *  short percussive noiseBurst events keep white noise for snap. */
  function pinkNoiseBuffer(audioCtx: AudioContext): AudioBuffer {
